#include "../yacl.h"
#include <stdint.h>
#include <stdlib.h>
#include <assert.h>
#include "../src/ecc/uECC.h"
#include <stdio.h>
#include <glib.h>

#define NUM_TESTS 1

uint8_t *keys[NUM_TESTS];
uint8_t *msg[NUM_TESTS];
uint8_t *mac[NUM_TESTS];

static void
t_hmac_rfc_4868_auth256_4 (void)
{
    uint8_t key [] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
                      0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10,
                      0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
                      0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20 };

    static unsigned char msg[50]
        = { 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd,
            0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd,
            0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd,
            0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd,
            0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd, 0xcd };


    uint8_t mac [] = {0x37, 0x2e, 0xfc, 0xf9, 0xb4, 0x0b, 0x35, 0xc2,
                      0x11, 0x5b, 0x13, 0x46, 0x90, 0x3d, 0x2e, 0xf4,
                      0x2f, 0xce, 0xd4, 0x6f, 0x08, 0x46, 0xe7, 0x25,
                      0x7b, 0xb1, 0x56, 0xd3, 0xd7, 0xb3, 0x0d, 0x3f };

    int rc;
    uint8_t result[YACL_SHA256_LEN];

    rc = yacl_hmac_sha256(key, sizeof(key),
                          msg, sizeof(msg),
                          result)
        ;

    g_assert (0 == rc);
    printf ("\n");
    yacl_hexdump (result, 32);

    g_assert (0 == yacl_memcmp (result, mac, sizeof(mac)));


}
static void
t_hmac_kat(void)
{

    uint8_t key[] =
        {0x97, 0x79, 0xd9, 0x12, 0x06, 0x42, 0x79, 0x7f,
         0x17, 0x47, 0x02, 0x5d, 0x5b, 0x22, 0xb7, 0xac,
         0x60, 0x7c, 0xab, 0x08, 0xe1, 0x75, 0x8f, 0x2f,
         0x3a, 0x46, 0xc8, 0xbe, 0x1e, 0x25, 0xc5, 0x3b,
         0x8c, 0x6a, 0x8f, 0x58, 0xff, 0xef, 0xa1, 0x76 };



    uint8_t msg[] =
        { };
    uint8_t mac[] =
        {0x76, 0x9f, 0x00, 0xd3, 0xe6, 0xa6, 0xcc, 0x1f, 0xb4, 0x26, 0xa1,
              0x4a, 0x4f, 0x76, 0xc6, 0x46, 0x2e, 0x61, 0x49, 0x72, 0x6e, 0x0d,
              0xee, 0x0e, 0xc0, 0xcf, 0x97, 0xa1, 0x66, 0x05, 0xac, 0x8b };

    int rc;
    uint8_t result[YACL_SHA256_LEN];

    rc = yacl_hmac_sha256(key, sizeof(key),
                          msg, sizeof(msg),
                          result)
;

    g_assert (0 == yacl_memcmp (result, mac, sizeof(mac)));
    g_assert (0 == rc);
}



int main(int argc, char *argv[])
{
    g_test_init (&argc, &argv, NULL);

    g_test_add_func ("/hmac/kat-rfc4868", t_hmac_rfc_4868_auth256_4);
    g_test_add_func ("/hmac/kat", t_hmac_kat);



    return g_test_run ();
}
